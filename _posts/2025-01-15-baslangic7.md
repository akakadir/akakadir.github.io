...
Çevrimdışı Mod seçeneğinde ZipRipper, yerel bir kaynak dosyası ````zr-offline.txt```` kullanır. ````zr-offline.txt```` dosyasının betik ile aynı klasörde bulunması gerekir ve çevrimdışı mod zorunlu hale gelir. Bu mod için internet bağlantısı gerekmez. (Portable kullanım için ideal)
