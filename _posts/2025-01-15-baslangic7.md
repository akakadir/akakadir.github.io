...
Yükleme tamamlandığında karşına bir uyarı daha gelecek.(bkz: [break](https://files.catbox.moe/y33729.png)) Bu temel olarak bir split(bölüm) uyarısıdır. elindeki Wordlist(sözcük kaynağı) fazla büyük ise ````yes```` seçeneğini kullanabilirsin. ````no```` seçeneğinde varsayılan modda atak başlatılır.
