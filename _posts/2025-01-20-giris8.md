...
Spotify API, dinamik içerik göstermek ya da Spotify hesabındaki bilgilere erişmek için kullanılabilecek bir araçtır. Örneğin, sitende anlık olarak dinlediğin şarkıyı göstermek istiyorsan API tam sana göre. Başlamak için [şu](https://developer.spotify.com) sayfasına git ve bir uygulama oluştur. Bu işlem sonunda sana bir ````Client ID```` ve ````Client Secret```` verilecek. Bunlar, API ile iletişim kurarken kimliğini doğrulamak için gereklidir.
